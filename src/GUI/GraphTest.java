...
public class GraphTest {
	...
	static void GraphTestGr() {
		GraphPanel graphPanel = new GraphPanel();
		GraphControl control = new GraphControl(graphPanel);
		JFrame f = new JFrame();
		f.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		f.getContentPane().add(control.getValuePanel(), "North");
		f.getContentPane().add(graphPanel);
		f.getContentPane().add(control.getUIPanel(), "South");
		f.pack();
		f.setLocationRelativeTo(null);
		f.setVisible(true);
	}
	
	static void GraphTest3D() {
		...
		Point3f b = new Point3f(0.0f, 0.721f, 0.0f); // botom
